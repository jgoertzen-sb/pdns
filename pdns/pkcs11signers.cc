...
static std::map<unsigned int, CK_MECHANISM_TYPE> dnssec2cmech = boost::assign::map_list_of(5, CKM_RSA_PKCS_KEY_PAIR_GEN)(7, CKM_RSA_PKCS_KEY_PAIR_GEN)(8, CKM_RSA_PKCS_KEY_PAIR_GEN)(10, CKM_RSA_PKCS_KEY_PAIR_GEN)(13, CKM_ECDSA_KEY_PAIR_GEN)(14, CKM_ECDSA_KEY_PAIR_GEN);

using CkaValueType = enum { Attribute_Byte,
                            Attribute_Long,
                            Attribute_String };
...
private:
  ...
  [[nodiscard]] unsigned int ecparam2bits(const std::string& obj) const
  {
    ...
    return bits;
#else
    if (d_ecdsa_params == ECDSA256_PARAMS)
      return 256;
    else if (d_ecdsa_params == ECDSA384_PARAMS)
      return 384;
    else
      throw PDNSException("Unsupported EC key");
#endif
...
public:
  ...
  int Sign(const std::string& data, std::string& result, CK_MECHANISM_PTR mechanism)
  {
    ...
    if ((d_err = slot->f()->C_SignInit(slot->Session(), mechanism, d_private_key))) {
      logError("C_SignInit");
      return d_err;
    }
  ...
  int Verify(const std::string& data, const std::string& signature, CK_MECHANISM_PTR mechanism)
  {
    ...
    if ((d_err = slot->f()->C_VerifyInit(slot->Session(), mechanism, d_public_key))) {
  ...
  int Digest(const std::string& data, std::string& result, CK_MECHANISM_PTR mechanism)
  {
    ...
    auto slot = d_slot->lock();
    if ((d_err = slot->f()->C_DigestInit(slot->Session(), mechanism))) {
...
void PKCS11DNSCryptoKeyEngine::create(unsigned int bits)
{
  ...
  if (mech.mechanism == CKM_RSA_PKCS_KEY_PAIR_GEN) {
    ...
    pubAttr.push_back(P11KitAttribute(CKA_LABEL, d_pub_label));
    if (d_algorithm == 13)
      pubAttr.push_back(P11KitAttribute(CKA_ECDSA_PARAMS, ECDSA256_PARAMS));
    else if (d_algorithm == 14)
      pubAttr.push_back(P11KitAttribute(CKA_ECDSA_PARAMS, ECDSA384_PARAMS));
    else
      throw PDNSException("pkcs11: unknown algorithm " + std::to_string(d_algorithm) + " for ECDSA key pair generation");
