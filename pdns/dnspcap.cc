...
PcapPacketReader::PcapPacketReader(const string& fname) :
  d_fname(fname)
{
  ...
  if (d_pfh.linktype == 1) {
    ...
    throw runtime_error((boost::format("Unsupported link type %d") % d_pfh.linktype).str());
  ...
  if (d_skipMediaHeader > d_bufsize)
...
PcapPacketWriter::PcapPacketWriter(const string& fname, const PcapPacketReader& ppr) :
  PcapPacketWriter(fname)
{
  setPPR(ppr);
}

PcapPacketWriter::PcapPacketWriter(const string& fname) :
  d_fname(fname)
{
