...
struct SendReceive
{
  ...
  SendReceive(map<ComboAddress, namecount, ComboAddress::addressOnlyLessThan>& res) :
    d_res(res)
  {
...
struct SendReceiveRes
{
  ...
  SendReceiveRes(const ComboAddress& remote, map<DNSName, vector<ComboAddress>>& out) :
    d_out(out)
  {
    d_socket = socket(AF_INET, SOCK_DGRAM, 0);
    int val = 1;
