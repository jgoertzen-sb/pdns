...
struct noinit_adaptor : Allocator
{
  ...
  struct rebind
  {
    typedef noinit_adaptor<typename std::allocator_traits<Allocator>::template rebind_alloc<U>> other;
  };
  ...
  noinit_adaptor(U&& u) noexcept :
    Allocator(std::forward<U>(u)) {}

  template <class U>
  noinit_adaptor(const noinit_adaptor<U>& u) noexcept :
    Allocator(static_cast<const U&>(u)) {}
...
template <class T>
using NoInitVector = std::vector<T, noinit_adaptor<std::allocator<T>>>;
