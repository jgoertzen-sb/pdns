...
public:
  IPSeqObfuscator() :
    d_romap(d_ipmap), d_ro6map(d_ip6map), d_counter(0)
  {
  }
...
{
public:
  IPCipherObfuscator(const std::string& key, bool decrypt) :
    d_key(key), d_decrypt(decrypt)
  {
    if (d_key.size() != 16) {
...
try {
  ...
  desc.add_options()("help,h", "produce help message")("version", "show version number")("key,k", po::value<string>(), "base64 encoded 128 bit key for ipcipher")("passphrase,p", po::value<string>(), "passphrase for ipcipher (will be used to derive key)")("decrypt,d", "decrypt IP addresses with ipcipher");

  po::options_description alloptions;
  po::options_description hidden("hidden options");
  hidden.add_options()("infiles", po::value<vector<string>>(), "PCAP source file(s)")("outfile", po::value<string>(), "outfile");
  ...
  for (const auto& inf : g_vm["infiles"].as<vector<string>>()) {
    ...
    cerr << "Saw " << pr.d_correctpackets << " correct packets, " << pr.d_runts << " runts, " << pr.d_oversized << " oversize, " << pr.d_nonetheripudp << " unknown encaps" << endl;
  }
}
