...
public:
  ...
  includeboilerplate(NAPTR) template <class Convertor>
  void xfrRecordContent(Convertor& conv);
...
public:
  ...
  includeboilerplate(A) void doRecordCheck(const DNSRecord& dr);
...
{
public:
  includeboilerplate(NS) explicit NSRecordContent(const DNSName& content) :
    d_content(content) {}
...
{
public:
  includeboilerplate(PTR) explicit PTRRecordContent(const DNSName& content) :
    d_content(content) {}
...
public:
  includeboilerplate(CNAME)
    CNAMERecordContent(const DNSName& content) :
    d_content(content) {}
...
public:
  includeboilerplate(DNAME)
    DNAMERecordContent(const DNSName& content) :
    d_content(content) {}
...
public:
  ...
  includeboilerplate(OPT) void getData(vector<pair<uint16_t, string>>& opts) const;
...
public:
  ...
  {
    return std::tie(d_flags, d_protocol, d_algorithm, d_key) < std::tie(rhs.d_flags, rhs.d_protocol, rhs.d_algorithm, rhs.d_key);
  }
};
...
public:
  ...
  {
    return std::tie(d_tag, d_algorithm, d_digesttype, d_digest) < std::tie(rhs.d_tag, rhs.d_algorithm, rhs.d_digesttype, rhs.d_digest);
  }
...
public:
  ...
  NSECBitmap(const NSECBitmap& rhs) :
    d_set(rhs.d_set)
  {
    if (rhs.d_bitset) {
...
public:
  ...
  includeboilerplate(APL) private : std::vector<APLRDataElement> aplrdata;
...
{
public:
  includeboilerplate(URI) private : uint16_t d_priority, d_weight;
  string d_target;
};
...
{
public:
  includeboilerplate(CAA) private : uint8_t d_flags;
...
#define boilerplate_conv(RNAME, CONV)                                                                               \
  boilerplate(RNAME) template <class Convertor>                                                                     \
  void RNAME##RecordContent::xfrPacket(Convertor& conv, bool /* noDot */)                                           \
  {                                                                                                                 \
    CONV;                                                                                                           \
    if (conv.eof() == false)                                                                                        \
      throw MOADNSException("When parsing " #RNAME " trailing data was not parsed: '" + conv.getRemaining() + "'"); \
  }                                                                                                                 \
  template <class Convertor>                                                                                        \
