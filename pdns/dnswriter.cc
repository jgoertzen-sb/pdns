...
template <typename Container>
void GenericDNSPacketWriter<Container>::startRecord(const DNSName& name, uint16_t qtype, uint32_t ttl, uint16_t qclass, DNSResourceRecord::Place place, bool compress)
{
  d_compress = compress;
  commit();
...
template <typename Container>
void GenericDNSPacketWriter<Container>::addOpt(const uint16_t udpsize, const uint16_t extRCode, const uint16_t ednsFlags, const optvect_t& options, const uint8_t version)
{
  uint32_t ttl = 0;
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfr48BitInt(uint64_t val)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrNodeOrLocatorID(const NodeOrLocatorID& val)
{
  d_content.insert(d_content.end(), val.content, val.content + sizeof(val.content));
}

template <typename Container>
void GenericDNSPacketWriter<Container>::xfr32BitInt(uint32_t val)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfr16BitInt(uint16_t val)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfr8BitInt(uint8_t val)
{
...
/* input:
  ...
  */
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrText(const string& text, bool, bool lenField)
{
  if (text.empty()) {
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrUnquotedText(const string& text, bool lenField)
{
  if (text.empty()) {
    d_content.push_back(0);
    return;
...
template <typename Container>
uint16_t GenericDNSPacketWriter<Container>::lookupName(const DNSName& name, uint16_t* matchLen)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrName(const DNSName& name, bool compress, bool)
{
  if (l_verbose)
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrBlob(const string& blob, int)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::xfrBlob(const std::vector<uint8_t>& blob)
{
  d_content.insert(d_content.end(), blob.begin(), blob.end());
}

template <typename Container>
void GenericDNSPacketWriter<Container>::xfrBlobNoSpaces(const string& blob, int)
{
  xfrBlob(blob);
}

template <typename Container>
void GenericDNSPacketWriter<Container>::xfrHexBlob(const string& blob, bool /* keepReading */)
{
  xfrBlob(blob);
}

template <typename Container>
void GenericDNSPacketWriter<Container>::xfrSvcParamKeyVals(const std::set<SvcParam>& kvs)
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::getRecordPayload(string& records)
{
  records.assign(d_content.begin() + d_sor, d_content.end());
}

template <typename Container>
uint32_t GenericDNSPacketWriter<Container>::size() const
{
  return d_content.size();
}

template <typename Container>
void GenericDNSPacketWriter<Container>::rollback()
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::truncate()
{
...
template <typename Container>
void GenericDNSPacketWriter<Container>::commit()
{
  if (!d_sor)
    return;
...
template <typename Container>
size_t GenericDNSPacketWriter<Container>::getSizeWithOpts(const optvect_t& options) const
{
