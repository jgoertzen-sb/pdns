...
RecordTextReader::RecordTextReader(string str, DNSName zone) :
  d_string(std::move(str)), d_zone(std::move(zone)), d_pos(0)
{
  /* remove whitespace */
...
void RecordTextReader::xfrTime(uint32_t& val)
{
  ...
  if (sscanf(tmp.str().c_str(), "%04d%02d%02d"
                                "%02d%02d%02d",
...
void RecordTextReader::xfrCAWithoutPort(uint8_t version, ComboAddress& val)
{
  if (version == 4) {
    ...
    throw RecordTextException("invalid address family");
}
...
RecordTextWriter::RecordTextWriter(string& str, bool noDot) :
  d_string(str)
{
  d_string.clear();
  d_nodot = noDot;
...
void RecordTextWriter::xfrTime(const uint32_t& val)
{
  ...
  static const boost::format fmt("%04d%02d%02d"
                                 "%02d%02d%02d");
